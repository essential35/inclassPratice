...
#要回存到（->classSurvey$性別）,classSurvey的性別欄形式才會變成factor
```
...
```{r ans132}
classSurvey[(isFemale == "TRUE"),]->subsample
```
...
#aaa==NA->因為等於NA（未定態），所以表示所有的字元都是未定態，所以跑出來為"NA NA NA"
```
...
```{r ans15}
#correct
(rowSums(hwGrades[2:4],na.rm = T)/3)->hwGrades$平均成績
hwGrades
```
...
```{r ans16}
hwGrades[(hwGrades$學號 %in% classSurvey$學號) ,]->hwGradesSubsample1
hwGradesSubsample1
...
```{r ans17}
#correct
classSurvey[isFemale,] %>%
{hwGradesSubsample$學號 %in% .$學號} ->hwGradesSubsample$女性
...
```{r ans18}
(hwGradesSubsample$女性 == "FALSE") %>%
  {colMeans(hwGradesSubsample[.,6]) }
...
```{r ans19}
#correct
(subsample$學號 %in% hwGradesSubsample$學號 ) %>%
  {subsample[.,]} %>%
  {str_detect(.$課外活動,"團隊活動")} %>%
  {colMeans(hwGradesSubsample[.,6])}
...
difftime(deadline,hwSubmit3$Timestamp,units = "hours")->hwSubmit3$截止時差
hwSubmit3
```
...
```{r ans23}
hwSubmit3[(hwSubmit3$女性 == "TRUE"),] %>%
{mean(.$截止時差)}->girl

hwSubmit3[(hwSubmit3$女性 == "FALSE"),] %>%
{mean(.$截止時差)}->boy
